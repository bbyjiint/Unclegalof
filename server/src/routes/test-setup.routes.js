...
/**
 * Test setup endpoint - creates a test business and user for testing
 * POST /api/test-setup
 * 
 * This endpoint is for development/testing only and should be disabled in production
 */
router.post("/", async (req, res, next) => {
  try {
    ...
    // Range represents the zone number, cost is the delivery fee
    const deliveryFees = [
    ...
    res.json({
      ...
      token,
      // Include this token in Authorization header: Bearer <token>
    });
  } catch (error) {
    next(error);
  }
